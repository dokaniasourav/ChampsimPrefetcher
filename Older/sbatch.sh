...
do
  ...
  do
    ind=$((j*3 + (k-1)))
    config_name="${configs[$j]}$k"
    echo ""
    echo "Starting job submissions for $config_name ..., index = $ind"
    ...
    do
      echo ""
      echo "Submitting job for the file ${files[$i]} with $config_name configuration and index = $ind"
      sbatch job_submit.sh "$i" "$config_name" "${desc[$ind]}" "$ind"
    done
    echo "Completed submissions for index $ind, $config_name"
    echo ""
done
done