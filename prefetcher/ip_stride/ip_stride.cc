...
struct tracker_entry {
  ...
  int64_t last_cl_addr = 0;    // the last address accessed by this IP
...
void CACHE::prefetcher_initialize() {
    std::cout << NAME << " IP-based stride prefetcher, Sets = " << TRACKER_SETS << " ways = " << TRACKER_WAYS << std::endl;
}

static record_str recorder[RECORD_SIZE];
uint64_t recorder_index = 0;
static int num_cycles = 0;

void CACHE::prefetcher_cycle_operate()
{
    auto [old_pf_address, stride, degree] = lookahead[this];
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in)
{
  ...
  /*
   * Tracker begins at the instruction pointer and ends at ip+4
   * Basically doing associative search for all the ways. Here no_of_ways = 4
   *
   * */
  ...
  if (found != set_end) {
    ...
    recorder[recorder_index%RECORD_SIZE] = {addr, stride, recorder_index, cache_hit, type};
  } else {
    // replace by LRU
...
void CACHE::prefetcher_final_stats() {
    std::cout << NAME << " Final Cache status message haha" << std::endl;
    std::cout << "Value of Rec = " << RECORD_SIZE << std::endl;
    for(int i=0; i < RECORD_SIZE; i++) {
        if(recorder[i].ip == 0)
            continue;
        std::cout << "IP = " << recorder[i].ip << ", "
        << "Stride = " << recorder[i].stride << ", "
        << "Rec Index = " << recorder[i].rec_index << ", "
        << "Cache Hit = " << recorder[i].cache_hit << ", "
        << "Type = " << recorder[i].type << ", " << std::endl;
    }
}
