...
void CACHE::prefetcher_cycle_operate() 
{
  ...
  if (auto [old_pf_address, degree, ip] = lookahead[this]; degree > 0) 
  {
...
void retrain_ppf(transfer_buff_entry* trans_buff, int ptr_to_trans_buff, int useful)
{
  ...
  index_degree = trans_buff[ptr_to_trans_buff].pf_degree - 1;	// index_degree is 1 less than the actual degree, e.g., for degree=1, it should index entry number 0
