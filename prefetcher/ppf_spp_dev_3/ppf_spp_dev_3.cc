#include <iostream>
#include <cstdint>
#include "ppf_spp_dev_3.h"
#include "ppf_setup_3.h"
#include "cache.h"
...
void CACHE::prefetcher_initialize() {
    ...
    oss << std::put_time(&tm, "./data/ppf-spp-dev_3_%Y%m%d%H%M%S_");
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip,
    ...
    if (cache_hit) {
        if(inverted_address.find(addr) != inverted_address.end()) {
            for(auto & ind: inverted_address[addr]) {
        ...
        for (uint32_t i = pf_q_head; i < pf_q_tail; i++) {
            if (confidence_q[i] >= PF_THRESHOLD) {
                ...
                if ((addr & ~(PAGE_SIZE - 1)) == (pf_address & ~(PAGE_SIZE - 1))) {
                    ...
                    entry_values.index_inst_add = ip ^ (last_sig<<4);
                    entry_values.index_misc_add = ((delta_q[i]<<6) ^ (confidence_q[i] << 2));
                    entry_values.index_misc_val = (lookahead_conf << 4) + (pf_q_tail << 2) + depth;
