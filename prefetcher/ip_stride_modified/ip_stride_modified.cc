...
void CACHE::prefetcher_initialize() { std::cout << NAME << " Modified IP-based stride prefetcher" << std::endl; }
...
void CACHE::prefetcher_cycle_operate()
{
  ...
  if (auto [old_pf_address, stride, degree] = lookahead[this]; degree > 0) {
    ...
    auto pf_address = old_pf_address + stride;		// modification done by Pushp and Aroma
